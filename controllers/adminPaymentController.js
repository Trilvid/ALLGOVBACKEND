...
exports.updatePaymentStatus = async (req, res) => {
  try {
    ...
    payment.metadata = {
      ...payment.metadata,
      adminNote: note,
      statusUpdatedBy: req.user?._id || req.userId,
      statusUpdatedAt: new Date(),
      previousStatus
    };
...
exports.processRefund = async (req, res) => {
  try {
    ...
    console.log('🔄 Processing refund for payment:', req.params.id);
    ...
    console.log('📄 Payment found:', payment.taxPaymentId);
    console.log('👤 User:', payment.userId.email);
    ...
    if (payment.metadata?.refunded) {
      console.error('❌ Payment already refunded');
      return res.status(400).json({
        success: false,
        message: 'This payment has already been refunded'
      });
    ...
    console.log('💰 User current balance:', oldBalance);

    // CRITICAL FIX: Use 'balance' field (not walletBalance)
    user.balance = oldBalance + amountToRefund;
    console.log('💰 New balance will be:', user.balance);

    // CRITICAL FIX: Mark balance as modified for Mongoose
    user.markModified('balance');

    // CRITICAL FIX: Add refund transaction to user's transaction history
    user.transaction.push({
      amount: amountToRefund,
      type: 'deposit',
      status: 'completed',
      reference: `REFUND-${payment.reference || payment.taxPaymentId}`,
      description: `Refund for ${payment.taxType} payment${reason ? `: ${reason}` : ''}`,
      date: new Date().toLocaleDateString(),
      timestamp: new Date()
    });

    // Mark transaction array as modified
    user.markModified('transaction');

    // Save user
    await user.save();
    console.log('💾 User balance updated and saved');

    // Verify the save worked
    const verifyUser = await User.findById(user._id).select('balance');
    ...
    payment.metadata = {
      ...
      refundAmount: amountToRefund,
      refundReason: reason,
      refundedBy: req.user?._id || req.userId,
      refundedAt: new Date(),
    ...
    try {
      ...
      console.error('⚠️ Notification error (refund still successful):', notifError);
    }
